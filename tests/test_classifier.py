...
def _make_synthetic_data(n=400):
    ...
    for _ in range(n // 4):
        rows.append({
            ...
            "bls_sde": rng.uniform(8, 30), "bls_snr": rng.uniform(15, 100),
            "bls_power": rng.uniform(0.01, 0.1), "period_agreement": rng.uniform(0, 0.05),
        })
    ...
    for _ in range(n // 4):
        rows.append({
            ...
            "bls_sde": rng.uniform(20, 100), "bls_snr": rng.uniform(100, 1000),
            "bls_power": rng.uniform(0.05, 0.5), "period_agreement": rng.uniform(0, 0.1),
        })
        labels.append(1)  # EB
    ...
    for _ in range(n // 4):
        rows.append({
            ...
            "bls_sde": rng.uniform(7, 25), "bls_snr": rng.uniform(8, 50),
            "bls_power": rng.uniform(0.01, 0.08), "period_agreement": rng.uniform(0.05, 0.3),
        })
    ...
    for _ in range(n // 4):
        rows.append({
            ...
            "bls_sde": rng.uniform(0.5, 9), "bls_snr": rng.uniform(1, 12),
            "bls_power": rng.uniform(0.001, 0.03), "period_agreement": rng.uniform(0.1, 1.0),
        })
...
def test_train_and_predict():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        pipeline, metrics = train(X, y, cv_folds=3, output_path=model_path)
    ...
    assert np.allclose(probs.sum(axis=1), 1.0, atol=1e-2)  # relaxed for calibrated model


def test_feature_importance():
    X, y = _make_synthetic_data(200)
    with tempfile.TemporaryDirectory() as tmpdir:
        model_path = Path(tmpdir) / "test_fi.pkl"
        pipeline, _ = train(X, y, cv_folds=2, output_path=model_path)
